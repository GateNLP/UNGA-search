  ...
  if (network_data) make_d3_clustergram(network_data);
...
function make_d3_clustergram(network_data) {
  ...
  network_data.links.forEach( function(link) {
    ...
    //copy the associated words into the matrix so we can re-build the query on click through
...
function row_function(row_data) {
  ...
  cell.append("text")
    ...
    .attr("x", function(d) { return x_scale(d.pos_x)+(x_scale.rangeBand()/2); })
    .attr("y", y_scale.rangeBand() / 2)
    .attr("dy", "0.5em")
...
function showSentences(d) {

	console.log(row_nodes[d.source].name+" --> "+ col_nodes[d.target].name);

	d3.select("#source").attr("value",row_nodes[d.source].name);
	d3.select("#target").attr("value",col_nodes[d.target].name);
	d3.select("#from").attr("value",network_data.from);
	d3.select("#to").attr("value",network_data.to);

	var form = d3.select("#sentences").node();
...
function reorder_click_row(d,i){
  ...
  t.selectAll(".cell-text")
    .attr('x', function(d){ 
    return x_scale(d.pos_x)+(x_scale.rangeBand()/2);
  });
